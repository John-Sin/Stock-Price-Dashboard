...
def get_tab1_info_box():
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Div(dcc.Input(id='tab1-ticker-input', value='XOM',
                                   type='text'),
                         style={'display': 'inline-block'}),
                html.Div(html.Button('Submit', id='tab1-submit'),
                         style={'display': 'inline-block'})
            ]),
            html.Div(id='tab1-error-message',
...
index_choice = [{'label': ' United States: S&P 500', 'value': 'sp500'},
                {'label': ' Hong Kong: Heng Seng Index', 'value': 'hsi'}]




 # data.append(go.Scatter(df['Date'], df['MA50'], df['MA100'], df['MA200']))
